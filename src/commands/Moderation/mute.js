   ...
   async run(msg, [member, when, reason]) {
      ...
      if (when) {
         ...
         return msg.sendMessage(
            `${this.client.config.emojis.success} ${member.user.tag} was temporarily muted for ${Duration.toNow(when)}${
      ...
      return msg.sendMessage(
         `${this.client.config.emojis.success} ${member.user.tag} was muted${reason ? ` with reason: **${reason}**` : '.'}`
      )
   }
}
