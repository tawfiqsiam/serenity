...
module.exports = class extends Command {
   ...
   async run(msg, [member, days = 0, ...reason]) {
      ...
      msg.sendMessage(
         new MessageEmbed().setColor(this.client.config.colors.default).setDescription(`${this.client.config.emojis.success} Banned ${member.user}`)
      );
      return new this.client.modlog(msg.guild).setType('ban').setModerator(msg.member).setUser(member).setReason({ current: reason }).send();
   }
};
