...
module.exports = class extends Command {
  ...
  async run(msg, [selected, ...reason]) {
    ...
    if (!channel) {
      msg.sendMessage(`${this.client.config.error} The mod log channel is invalid, contact a server administrator and ask them to re-set it.`)
    ...
    const message2 = messages.find(mes => mes.author.id === this.client.user.id &&
      mes.embeds.length > 0 &&
      mes.embeds[0].type === 'rich' &&
      mes.embeds[0].footer && mes.embeds[0].footer.text === `Case ${log.originalCase}`
    );
    ...
    if (message2) {
      ...
      ["mute", "unmute"].includes(log.type) && log.duration !== null ? embed.addField((log.type === "unmute") ? `**Original Mute Duration**` : `**Duration**`, `${log.duration[0].toUpperCase()}${log.duration.slice(1)}`) : ""
      log.type === "unmute" ? embed.addField(`**Original Mute Reason**`, `${log.originalReason ? log.originalReason : `No reason provided`}`) : ""
