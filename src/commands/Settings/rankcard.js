...
module.exports = class extends Command {
   ...
   async run(msg, [color]) {
      let check = msg.author.settings.get('colors.rankcard')
      if (color === 'reset') {
         if (check === '#7B2BC0') throw msg.language.get('COMMAND_RANKCARD_DEFAULT_EXISTS')
         await msg.author.settings.reset('colors.rankcard')
      ...
      await msg.author.settings.update('colors.rankcard', color)
