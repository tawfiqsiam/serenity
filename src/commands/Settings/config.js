...
module.exports = class extends Command {
   ...
   show(message, [key]) {
      ...
      let embed = new MessageEmbed().setDescription(this.displayFolder(value, key ? key : undefined)).setColor(this.client.config.colors.default);
         ...
         for (const keyType of [...sections.keys()].sort()) {
            array.push(
               ...sections
                  .get(keyType)
                  .sort()
                  .map((key) => `${key.padEnd(longest)} → ${this.displayEntry(settings.schema.get(key), settings.get(key), settings.base.target)}`)
            );
         }
      }
      return array.join('\n');
   ...
   displayEntry(entry, value, guild) {
      return entry.array ? this.displayEntryMultiple(entry, value, guild) : this.displayEntrySingle(entry, value, guild);
   }

   displayEntrySingle(entry, value, guild) {
      return entry.serializer.stringify(value ? value : 'None', guild);
   }

   displayEntryMultiple(entry, values, guild) {
      return values.length === 0 ? 'None' : `[ ${values.map((value) => this.displayEntrySingle(entry, value, guild)).join(' | ')} ]`;
   }
