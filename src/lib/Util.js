      ...
      try {
        for (const channel of guild.channels.values()) {
          if (channel.type === ("text" || "category")) {
            await channel.createOverwrite(role, { SEND_MESSAGES: false }, 'Muted role overwrites required.').catch(console.error)
          }
          if (channel.type === ("voice" || "category")) {
            await channel.createOverwrite(role, { CONNECT: false }, 'Muted role overwrites required.').catch(console.error)
          } else {
            continue;
          }
        }
