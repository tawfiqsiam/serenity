...
class Levels {
   ...
   async transferLevels(guild) {
      return new Promise(async (fulfill, reject) => {
         await request.get(`https://mee6.xyz/api/plugins/levels/leaderboard/${guild.id}?page=0&limit=999`, async (err, res, b) => {
            ...
            if (body.role_rewards && body.role_rewards.length !== 0) {
               for (const entry of body.role_rewards) {
                  let role = guild.roles.cache.get(entry.role);
                  if (!role || role === null) continue;
                  await this.addGuildReward(guild, role, entry.rank);
               }
            }
   ...
   async giveGuildUserExp(member, msg) {
      ...
      if (oldLvl != newLvl) {
         ...
         let getChannel = msg.guild.channels.cache.get(dbChannel);
            ...
            case 'embed':
               ...
               (await getChannel)
                  ? getChannel.send(levelUpEmbed).catch((err) => {
                       msg.guild.settings.reset('levels.rankup.channel');
   ...
   async updateUserRoles(member) {
      try {
         ...
         if (lvl === 0) return;
         let rewards = await this.getGuildRewards(member.guild);
         if (rewards.length > 0) {
            let rolesToAdd = [];
            for (const reward of rewards) {
               if (reward.level > lvl) return;
               if (!member.roles.cache.has(reward) && member.guild.me.roles.highest.rawPosition > reward.role.rawPosition) {
                  rolesToAdd.push(reward.role);
               }
            }
            try {
               if (!member.roles.cache.has(rolesToAdd)) {
