  ...
  async run(member) {
    ...
    if (channel) {
      ...
      if (member.guild.settings.get('toggles.welcomeimage')) {
        attachment = new MessageAttachment(await generateWelcome(member, await member.guild.settings.get('colors.welcomecard')), 'welcome-image.png');
      }
      ...
      if (!message) {
        ...
        var welcomeMessage = message.replace(/\%user/ig, member).replace(/\%tag/ig, member.user.tag)
      }
