...
module.exports = class extends Event {
   async run(role) {
      ...
      let flags = ''
      role.permissions.toArray().forEach((flag) => {
         flags = flags.concat(`${this.client.config.emojis.success} ${flag} \n`)
      })

      if (!role.guild.settings.get('toggles.logs.roles')) return
      if (!role.guild.me.permissions.has('VIEW_AUDIT_LOG') || !role.guild.me.permissions.has('MANAGE_WEBHOOKS')) return
      const embed = new MessageEmbed()
         .addField('Name', role.name)
         .addField('Position', role.position, true)
         .addField('Color', role.hexColor, true)
         .addField('Permissions', flags.slice(0, 1024))
